...
path_to_data = 'simple-economy/' # TODO path of training

model_name = 'ego-dem-aie' # TODO 'saez' or 'aie' or 'ego-dem-aie' or 'con-dem-aie-[utilitarian_weight]' (f.e. 'con-dem-aie-50')

model_desc = 'Ego-Democratic AI Economist' # TODO 'Saez Formula' or 'AI Economist' or 'Ego-Democratic AI Economist' or 'Considerate-Democratic AI Economist 50' (or similar -> need to add plot_cols)
    ...
    for idx in range(num_dense_logs):
        ...
        for t in range(env_obj.env.episode_length):
            ...
            for agent_idx in range(env_obj.env.n_agents):
                ...
                actions[str(agent_idx)] = trainer.compute_single_action(
                    obs[str(agent_idx)], 
            ...
            actions["p"] = trainer.compute_single_action(
                obs['p'], 
                planner_states, 
                policy_id='p',
                full_fetch=False
            ...
            obs, rew, done, _, info = env_obj.step(actions)        
            if done['__all__']:
                break
...
trainer = PPOConfig.from_dict(config_dict=trainer_config).environment(RLlibEnvWrapper).build()
...
def search_ckpts(rootdir):
    ...
    for _, _, files in os.walk(rootdir):
        for file in files:
            if(file.startswith("agent.tf.weights.global-step-")):
                file_list.append(file)
    return file_list

ckpts_list = search_ckpts(str(path_to_data) + 'ckpts/')
step_list = [int(elem.lstrip("agent.tf.weights.global-step-")) for elem in ckpts_list]
step_list.sort()
...
for step in step_list: # Iterate through all checkpoints
    ...
    for i in range(multiple_runs):
        ...
        for metric in results[i].keys():
            ...
            elif metric == 'Avg. Bracket Rate: 4000':
                results[i][metric].append(dummy_env.env.metrics['PeriodicTax/avg_bracket_rate/4000'])
