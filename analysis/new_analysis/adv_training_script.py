...
model_name = 'saez' # TODO 'saez' or 'aie' or 'ego-dem-aie' or 'con-dem-aie-[utilitarian_weight]' (f.e. 'con-dem-aie-50')

model_desc = 'Saez Formula' # TODO 'Saez Formula' or 'AI Economist' or 'Ego-Democratic AI Economist' or 'Considerate-Democratic AI Economist 50' (or similar -> need to add plot_cols)
...
if __name__ == "__main__":
    ...
    trainer_envs = remote.get_trainer_envs(trainer)
    ...
    training_results = [
            ...
            'Productivity': [],
            'Environment Steps': []
            } for _ in range(len(trainer_envs))]
    ...
    while num_parallel_episodes_done < run_config["general"]["episodes"]:
        ...
        logger.info(
            ...
            curr_iter,
            result["episodes_this_iter"] * result["episode_len_mean"],
            global_step,
            num_parallel_episodes_done,
        ...
        # logger.info(str(remote.get_trainer_envs(trainer)[0].env.metrics))
        ...
        for i in trainer_envs.keys():
            
            def extract_metrics(env_wrapper):
                return env_wrapper.env.metrics
            
            env_metrics = remote.remote_env_fun(trainer, extract_metrics)
            
            logger.info(env_metrics)
            
            training_results[i]['Environment Steps'].append(global_step)
            
            for metric in training_results[i].keys():
                if metric == 'Inverse-Income Weighted Utility':
                    training_results[i][metric].append(dummy_env.env.metrics['social_welfare/inv_income_weighted_utility'])
                elif metric == 'Income Equality * Productivity':
                    training_results[i][metric].append(dummy_env.env.metrics['social_welfare/coin_eq_times_productivity'])
                elif metric == 'Income Equality':
                    training_results[i][metric].append(dummy_env.env.metrics['social/equality'])
                elif metric == 'Productivity':
                    training_results[i][metric].append(dummy_env.env.metrics['social/productivity'])
    ...
    result_path = run_dir + "/real_training_history.pkl"
