...
class ConsiderateDemocraticOneStepEconomy(BaseEnvironment):
    """
    A simple model featuring one "step" of setting taxes and earning income. Modified from one_step_economy.

    As described in https://arxiv.org/abs/2108.02755: (but modified)
        Each agent chooses an amount of labor that optimizes
        ...
        the agents act, all agents set the marginal tax rates in order to optimize
        their best gain.

    Note:
        This scenario is intended to be used with the 'DemocraticPeriodicBracketTax' and
            'SimpleLabor' components.
        It should use an episode length of 2. In the first step, taxes are set by
            the agents via 'DemocraticPeriodicBracketTax'. In the second, agents select how much
    ...
    Args:
        ...
        utilitarian_weight (float): Must be between 0 and 1. How important the utilitarian part
            of an agents utility is to them. At 0 utility is just personal gain, at 1 utility is just social welfare.
    """

    name = "considerate-democratic-one-step-economy"
    ...
    def __init__(
        ...
        **base_env_kwargs
        ...
        self.utilitarian_weight = utilitarian_weight
        assert self.utilitarian_weight >= 0 and self.utilitarian_weight <= 1
    ...
    def get_current_optimization_metrics(
        ...
        # Optimization metric for agents: TODO !!!
        for agent in agents:
        ...
        utilitarian_utility = rewards.inv_income_weighted_utility( 
        ...
        for agent in agents:
            curr_optimization_metric[agent.idx] = self.utilitarian_weight * utilitarian_utility + (1-self.utilitarian_weight) * curr_optimization_metric[agent.idx]
            
        # Need a dummy for the planner
        curr_optimization_metric[self.world.planner.idx] = 1
